...
class SecondPlayerSettingsVC: UIViewController {
    var emojiList: [[String]] = []
        
    override func viewDidLoad() {
        super.viewDidLoad()
        fetchEmojis()
        playerSymbolLabel.text = emojiList[0][1]
...
extension SecondPlayerSettingsVC: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        playerSymbolLabel.text = emojiList[indexPath.section][indexPath.item]
    }
}

extension SecondPlayerSettingsVC: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: 50, height: 50)
    }
}

extension SecondPlayerSettingsVC: UICollectionViewDataSource {
